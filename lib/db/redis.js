...
lib_redis.close = function(__callback) {
    if (__config.redis.init) {
        __logger.warn('lib_redis.close, function called', { port: __config.redis.port, host: __config.redis.host });
        lib_redis.conn.quit();
        __callback(null);
    } else {
        __callback(null);
...
lib_redis.increment = function(key, value, __callback) {
    if (lib_redis.conn) {
        lib_redis.conn.incrby(key, value);
        if (__callback)
            __callback(null);
    } else {
        if (__callback)
...
lib_redis.hget_json = function(key, __callback) {
    if (lib_redis.conn) {
...
lib_redis.get_keys = function(key, __callback) {
    if (lib_redis.conn) {
        lib_redis.conn.keys(key, function(err, json_object) {
            if (err) {
                __callback(err);
            } else {
                __callback(null, json_object);
...
lib_redis.check_key_exists = function(key, __callback) {
    if (lib_redis.conn) {
        lib_redis.conn.exists(key, function(err, value) {
            if (err) {
                __callback(err);
            } else {
                __callback(null, value);
            }
        });
    } else {
        __callback(new Error('redis connection failed'));
